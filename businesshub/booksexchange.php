...
(function(){
  ...
  bookCards.forEach(card => {
    ...
    // 2) When Take modal opens, fetch offers
      ...
      fetch(`getOffers.php?book_id=${bookId}`)
        ...
        .then(offers => {
          ...
          offers.forEach(o => {
            ...
            `;
            // if it's mine, show DROP; else REQUEST
...
// 3b) Delegate Drop clicks
giverList.addEventListener('click', e => {
  if (!e.target.matches('.drop-btn')) return;
  ...
  .then(r => r.text())        // grab raw response
  .then(text => {
    console.log('raw drop.php response →', text);
    const data = JSON.parse(text);
    console.log('parsed JSON →', data);

    if (data.status === 'ok') {
      e.target.closest('li').remove();
      const card = document.querySelector(`.book-card[data-book-id="${data.book_id}"]`);
      if (card) {
