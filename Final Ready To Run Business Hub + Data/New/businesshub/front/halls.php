...
<head>
  ...
  <link rel="stylesheet" href="css/deps.css">
    ...
    <title>halls</title>
    <style>
/* highlight/dim classes from staff feature */
    .dimmed   { opacity: 0.3; transition: opacity 0.5s ease; }
    .focused  { opacity: 1 !important; background-color: #fff3a6; transition: background-color 0.5s ease; }
    /* hall-row styling */
    .hall-row.focused { background-color: #fff3a6 !important; }
/* HALLS STYLE 🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵 */

.halls-section {text-align: center; padding: 30px 10px;}
...
.highlighted-row {
  ...
  <main class="page-content">
    ...
    <div class="container hall-details" id="hallDetails" style="display:none">
      <h2 id="buildingName">Building Name</h2>
      <table class="table table-bordered">
        <thead class="table-dark">
          <tr>
            <th>اسم القاعة</th><th>الطابق</th><th>السعة</th><th>فيديو</th>
          </tr>
        </thead>
  ...
  <?php include __DIR__ . '/../includes/footer.php'; ?>
      ...
      },
      {
        ...
        halls: [
          ...
          { name: "قاعة الندوات الاقتصاد", floor: "الطابق الأرضي", capacity: 95, video: "videos/B2/seminar-biz2-gf.mp4" },
      ...
      },
      {
        ...
        halls: [
          ...
          { name: "قاعة الندوات اعمال 4", floor: "الطابق السفلي", capacity: 110, video: "videos/B4/seminar-biz4-basement.mp4" },
...
<!-- for hall search global -->
<script>
      // render building boxes
    function displayBuildings(){
      const c = document.getElementById("hallsContainer");
      c.innerHTML = "";
      buildings.forEach(b => {
        c.innerHTML += `
          <div class="hall-box" onclick="showBuildingDetails('${b.name}')">
            <div class="preview-container">
              <img src="${b.image}" alt="${b.name}">
              <video src="${b.previewVideo}" muted loop playsinline preload="auto" controlsList="nodownload"></video>
            </div>
    ...
    function showBuildingDetails(name){
      ...
      const tb = document.getElementById("hallTableBody");
      tb.innerHTML = "";
      b.halls.forEach(h=>{
        tb.innerHTML += `
          <tr class="hall-row" data-hall-name="${h.name}">
            <td>${h.name}</td>
            <td>${h.floor}</td>
            <td>${h.capacity}</td>
            <td><button onclick="playHallVideo(event,'${h.video}')">الطريق الى هناك</button></td>
          </tr>`;
      });
      document.getElementById("hallDetails").style.display = "block";
      document.getElementById("hallDetails").scrollIntoView({behavior:'smooth'});
    }

    function playHallVideo(ev, src){
  ev.stopPropagation();
  const row = ev.target.closest("tr");
  const next = row.nextElementSibling;

  // If this row already has its video open, close it:
  if(next && next.classList.contains('video-row')){
    next.remove();
    return;
  }

  // Otherwise remove any other open video rows:
  document.querySelectorAll('.video-row').forEach(r=>r.remove());

  // And open this one:
  const vrow = document.createElement('tr');
  vrow.className = 'video-row';
  vrow.innerHTML = `<td colspan="4">
    <video src="${src}"
           controls
           playsinline
           muted
  ...
  row.after(vrow);
  vrow.querySelector('video').play().catch(()=>{});
}
    document.addEventListener("DOMContentLoaded", ()=> {
      displayBuildings();

      // if ?highlightHall=… in URL…
      const p = new URLSearchParams(location.search);
      if(p.has("highlightHall")){
        const hallName = p.get("highlightHall");
        // find building, show details
        const b = buildings.find(b=>b.halls.some(h=>h.name===hallName));
        if(b) showBuildingDetails(b.name);
        // then highlight the row
        setTimeout(()=>{
          const row = document.querySelector(`tr[data-hall-name="${hallName}"]`);
          if(row){
            const all = document.querySelectorAll(".hall-row");
            all.forEach(r=>r.classList.add("dimmed"));
            row.classList.remove("dimmed");
            row.classList.add("focused");
            row.scrollIntoView({behavior:'smooth',block:'center'});
            setTimeout(()=>{
              all.forEach(r=>r.classList.remove("dimmed","focused"));
            }, 2000);
          }
        }, 200);

        // remove param so refresh won't re-trigger
        p.delete("highlightHall");
