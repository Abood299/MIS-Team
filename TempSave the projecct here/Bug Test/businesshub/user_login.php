...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SESSION['lockout_time']) && $current_time < $_SESSION['lockout_time']) {
        ...
        // Fetch user (including block flag)
        $sql = "
            SELECT 
              id, 
              first_name, 
        ...
        if ($auth_ok) {
            ...
            // Only count as a "bad attempt" if not blocked
            if (!$error) {
...
<head>
 ...
 /* Reset CSS */
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .left-content { position:absolute; top:0; left:0; z-index:4; padding:60px 40px; color:#fff; }
    ...
    form input[type="email"], form input[type="password"] {
      padding:12px 16px; border:1px solid #ddd; border-radius:4px; font-size:1rem; width:100%; transition:border-color 0.2s;
    }
    ...
    form button[type="submit"] { margin-top:8px; padding:14px; border:none; border-radius:4px; background-color:#5E2950; color:#fff; font-size:1rem; font-weight:500; cursor:pointer; transition:background-color 0.3s; }
    ...
    @media (max-width:850px) {
      ...
      .left-content { padding:40px 20px; text-align:center; }
    }
    /* apply same styling to email, password & text fields */
...
<body>
  <div class="split-screen">
    <div class="left">
      <img id="bg-bottom" src="images/sign1.jpg" class="bg-image" alt="">
      <img id="bg-top"    src="images/sign1.jpg" class="bg-image" alt="">
      <div class="overlay"></div>
      <div class="left-content">
        <h1>Business Hub</h1>
        <p>From study materials and hall guides to book exchanges and staff locations.
           Business Hub is your one-stop platform for smarter, faster academic life at the University of Jordan.</p>
...
<script>
    ...
    (function(){
      ...
      let idx         = 0;
      const topImg    = document.getElementById('bg-top');
      const bottomImg = document.getElementById('bg-bottom');

      setInterval(() => {
        // preload next into bottom
        idx = (idx + 1) % images.length;
        bottomImg.src = images[idx];

        // fade out top
        topImg.style.opacity = 0;

        // when fade ends, swap and reset
        topImg.addEventListener('transitionend', function handler() {
