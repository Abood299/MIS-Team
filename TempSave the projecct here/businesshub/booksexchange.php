...
document.getElementById('desired_major')
  .addEventListener('change', async function() {
    ...
    try {
      ...
      books.forEach(b => {
        if (String(b.id) === offeredId) return;    // ← skip it
...
// ——— 2) Load ALL swap offers into panel ———
        ...
        swaps.forEach(o => {
          ...
          li.innerHTML = `
            <div>
              ...
              <span class="offered-title">${o.offered_title}</span><br>
              wants <span class="desired-title">${o.desired_title}</span> in exchange<br>
              <small class="text-muted">
                ${new Date(o.offered_at).toLocaleDateString()}
              </small>
              <p class="mt-2"><strong>Details:</strong> ${o.offer_condition}</p>
            </div>
            <button class="btn btn-sm btn-primary request-swap-btn"
...
// 4) Delegate “Request Swap” clicks
document.getElementById('swapOffersList')
  .addEventListener('click', async e => {
    if (!e.target.matches('.request-swap-btn')) return;
    if (!window.isLoggedIn) return window.location='user_login.php';
